# -*- coding: UTF-8 -*-
import os, shutil, getpass, Tkinter, ttk, tkMessageBox, re, sys, stat
...
def p4Clients():
    ...
    for i in p4clientinfo:
        if i['client'].lower() == yourName + '_artsource':
            ...
            p4Engine_client.append(i['Root'])
        elif i['client'].lower() == yourName + '_fortnite_main':
            p4Engine_client.append(i['client'])
            p4Engine_client.append(i['Root'])
...
# P4ICON = ImageTk.PhotoImage(Image.open('P:\\Developers\\Yifei.Zhang\\Plugins\\EGC_Auto\\img\\img\\p4.ico'))
...
def selmaya():
    try:
        ...
        openMel.close()
        os.chmod(MayaMelfolder + '\\' + 'shelf_EGC_ExportFBX.mel', stat.S_IREAD)
        # Engine_Setuped = delcreate('EGC_Auto_Export_MAYA.uasset', Bakefolder)
        EGCForEGC = delcreate_folder('ForEGC', p4Clients()[1][1] + '\\FortniteGame')
        Version_Setuped = delcreate('Version.json', EGC_Plugins)
        EGC_Setuped = delcreate('EGC_AutoImEx.py', EGC_Plugins)
        tkMessageBox.showinfo(u'安装成功',
                              u'安装成功\n%s\n%s\n%s\n%s\n%s' % (EGC_Setuped, software_Setuped, Marmo_Setuped, EGCForEGC, Version_Setuped))
    except Exception as e:
        tkMessageBox.showerror(u'注意', u'请关闭MAX,MAYA和引擎\n%s' % e)
