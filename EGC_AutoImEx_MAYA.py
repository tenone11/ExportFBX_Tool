...
# update info
reload(EGC_configs)
fbxPath = EGC_configs.MayaExportedFile
...
if __name__ == "__main__":
    if os.path.isfile(configini) == True:
        if EGC_configs.MayaToUnreal == True:
            for i in fbxPath:
                EGC_imexport(i)
            out_file = open(configini, 'w')
            out_file.write('MayaToUnreal = False')
            out_file.write('\nUnrealExport = True')
            out_file.write('\nmayaExportedFile = None')
            out_file.close()
        else:
            print "no FBX from MAYA currently"